...
class Cache {
    ...
    initialize(options) {
        ...
        const init = (0, utils_js_1.compare)(options.init, null) ? null : options.init;
        ...
        for (const [index, value] of cached) {
            if ((0, utils_js_1.compare)(value.init, options.init)) {
                return {
                    url,
                    name,
                    index,
    ...
    findUri(cacheName, url) {
        ...
        const parsedCache = new mongodb_connection_string_url_1.ConnectionString(url);
        for (const [storedUrl] of namedCache) {
            const parsedUri = new mongodb_connection_string_url_1.ConnectionString(storedUrl);
            if ((0, utils_js_1.compareUris)(parsedUri, parsedCache)) {
                return storedUrl;
            }
        }
        return undefined;
    ...
    async waitFor(cacheIndex) {
        ...
        return new Promise((resolve, reject) => {
            const _resolve = (index) => {
                if ((0, utils_js_1.compare)(cacheIndex, index)) {
